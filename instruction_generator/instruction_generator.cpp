...
uint32_t generate_branch_instr(){
    ...
    int32_t immediate = rand() % (int32_t)pow(2,12);
    int32_t imm_field1 = immediate & 0x00000800;
    int32_t imm_field2 = immediate & 0x000003F0;
    int32_t imm_field3 = immediate & 0x0000000F;
    int32_t imm_field4 = immediate & 0x00000400;
    int32_t final_immediate = imm_field1 << 31 | imm_field2 << 25 | imm_field3 << 8 | imm_field4 << 7;
    uint32_t instruction = final_immediate | rs2_idx << 20 | rs1_idx << 15 | funct3 << 12 | opcode;
    return instruction;
}

uint32_t generate_jal_instr(){
    uint32_t opcode = 0b1101111;
    uint32_t rd_idx = rand() % 31 + 1; //no zeroes
    int32_t immediate = rand() % (int32_t)pow(2,20);
    int32_t imm_field1 = immediate & 0x00080000;
    int32_t imm_field2 = immediate & 0x000003FF;
    int32_t imm_field3 = immediate & 0x00000400;
    int32_t imm_field4 = immediate & 0x0007F800;
    int32_t final_immediate = imm_field1 << 31 | imm_field2 << 21 | imm_field3 << 20 | imm_field4 << 12;
